...
export default function PostSaleChecklist({ lead, compact = false }) {
  ...
  if (loading && tasks.length === 0) {
    return <p style={{ fontSize: 13, color: '#6b7280', padding: 12 }}>Loading handover steps…</p>;
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {tasks.map((t) => {
          ...
          return (
            <div key={t.id} style={{ background: isDone ? '#f9fafb' : '#fff', border: '1px solid #e5e7eb', borderRadius: 10, padding: '10px 12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <span style={{ fontSize: 13, fontWeight: 600, color: isDone ? '#9ca3af' : '#111827', textDecoration: isDone ? 'line-through' : 'none', display: 'block', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {meta.label}
                    </span>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 3, flexWrap: 'wrap' }}>
                      {t.owner_role && (
                        <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.04em', textTransform: 'uppercase', color: '#9ca3af' }}>
                          {OWNER_LABELS[t.owner_role] || t.owner_role}
                        </span>
                      )}
                      {t.cost != null && (
                        <span style={{ fontSize: 10, color: '#6b7280' }}>RM {Number(t.cost).toLocaleString()}</span>
                      )}
                    </div>
                  </div>
              ...
              {isOpen && (
                <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <p style={{ fontSize: 11.5, lineHeight: 1.5, color: '#6b7280', margin: 0 }}>{meta.hint}</p>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                    {/* Quick status overrides for the less-common states */}
                    <button onClick={() => updateTask(t.id, { status: 'in_progress' })}
                      style={{ fontSize: 10, fontWeight: 600, padding: '5px 10px', borderRadius: 6, cursor: 'pointer', color: t.status === 'in_progress' ? '#d97706' : '#6b7280', background: t.status === 'in_progress' ? 'rgba(217,119,6,0.1)' : '#f3f4f6', border: '1px solid #e5e7eb' }}>
                      In progress
                    </button>
                    {meta.optional && (
                      <button onClick={() => updateTask(t.id, { status: t.status === 'na' ? 'pending' : 'na' })}
                        style={{ fontSize: 10, fontWeight: 600, padding: '5px 10px', borderRadius: 6, cursor: 'pointer', color: t.status === 'na' ? '#d97706' : '#6b7280', background: '#f3f4f6', border: '1px solid #e5e7eb' }}>
                        {t.status === 'na' ? 'Mark applicable' : 'Not applicable'}
                      </button>
                    )}
                    <label style={{ fontSize: 10, color: '#9ca3af', display: 'inline-flex', alignItems: 'center' }}>Due
                      <input type="date" value={t.due_date || ''} onChange={(e) => updateTask(t.id, { due_date: e.target.value || null })}
                        style={{ marginLeft: 6, fontSize: 11, padding: '5px 7px', borderRadius: 6, background: '#fff', border: '1px solid #d1d5db', color: '#111827' }} />
                    </label>
                    <label style={{ fontSize: 10, color: '#9ca3af', display: 'inline-flex', alignItems: 'center' }}>Cost RM
                      <input type="number" value={t.cost ?? ''} onChange={(e) => updateTask(t.id, { cost: e.target.value === '' ? null : Number(e.target.value) })}
                        style={{ marginLeft: 6, width: 80, fontSize: 11, padding: '5px 7px', borderRadius: 6, background: '#fff', border: '1px solid #d1d5db', color: '#111827' }} />
                    </label>
                  </div>
                  {!compact && (
                    <input type="text" placeholder="Notes…" defaultValue={t.notes || ''} onBlur={(e) => updateTask(t.id, { notes: e.target.value })}
                      style={{ fontSize: 11.5, padding: '7px 9px', borderRadius: 6, background: '#fff', border: '1px solid #d1d5db', color: '#111827' }} />
                  )}
                </div>
              )}
            </div>
      ...
      {(() => {
        ...
        return (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', borderRadius: 8, background: '#f9fafb', border: '1px solid #e5e7eb' }}>
            <span style={{ fontSize: 11.5, color: '#6b7280' }}>Processing cost</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: '#111827' }}>RM {total.toLocaleString()}</span>
          </div>
        );
      })()}
    </div>
