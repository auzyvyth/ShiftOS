...
import { Send, MessageSquare, ArrowRight, UserCheck, Clock, Plus } from 'lucide-react';
import { STAGE_CONFIG } from '../../lib/leadsHelpers';

const TYPE_CONFIG = {
  created:       { Icon: Plus,        color: 'text-sky-400',     bg: 'bg-sky-500/10',     label: 'Lead created' },
  stage_changed: { Icon: ArrowRight,  color: 'text-violet-400',  bg: 'bg-violet-500/10',  label: 'Stage changed' },
  note_added:    { Icon: MessageSquare, color: 'text-amber-400', bg: 'bg-amber-500/10',   label: 'Note' },
  assigned:      { Icon: UserCheck,   color: 'text-emerald-400', bg: 'bg-emerald-500/10', label: 'Assigned' },
};
...
function ActivityItem({ activity }) {
  ...
  if (activity.activity_type === 'created')       desc = 'Lead added to pipeline';
    ...
    const from = STAGE_CONFIG[activity.from_stage]?.label || activity.from_stage;
  ...
  return (
    <div className="flex gap-3 group">
      ...
      <div className="pb-4 flex-1 min-w-0">
        ...
        {desc && <p className="text-xs text-gray-400 leading-relaxed break-words">{desc}</p>}
        {activity.creator?.full_name && (
          <p className="text-xs text-gray-600 mt-0.5">by {activity.creator.full_name}</p>
        )}
      </div>
    </div>
  );
...
export default function LeadActivityLog({ activities, onAddNote, loading }) {
  ...
  return (
    <div>
      <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-4">Activity</h4>
