...
export default function SalesmanPremium() {
 ...
 // type + ref_id are what turn a row into a link to the thing it is about.
 // They were always on the table; not selecting them is why every
...
? { ...l, stage: "lost", loss_reason: reason, updated_at: now }
 ...
 if (error) setNotifications((p) => p.map((n) => (fresh.includes(n.id)? { ...n, is_read: false } : n)));
 };
...
? `RM ${Number(car.selling_price).toLocaleString("en-MY")}`
 ...
 // NOTIFICATION PANEL — markup lives in the shared component (Lite renders the
 // same one). This page only owns where each notification goes.
...
? `Hi ${lead.buyer_name || "kawan"}! Ada orang lain tengah tanya pasal ${waCarName} ni — kalau you still interested, jom lock dulu sebelum terlambat `
 ...
 pipeline lead drawer's car block. Was pure static text/image here, so
 a booking's car never linked to anything. */}
 <div
