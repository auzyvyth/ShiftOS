...
export default function SalesmanPremium() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data, error }) => {
      ...
      // Only premium standalone accounts belong here
        ...
        >
          ...
          >
            ...
            >
              ...
              {leadScores[lead.id] && (
                <span
                  title={leadScores[lead.id].reason}
                  style={{
                    fontSize: 9, fontWeight: 700, padding: "1px 6px", borderRadius: 99, lineHeight: 1.4, whiteSpace: "nowrap",
                    background: leadScores[lead.id].score === "hot" ? "rgba(239,68,68,0.15)" : leadScores[lead.id].score === "warm" ? "rgba(251,191,36,0.12)" : "rgba(96,165,250,0.12)",
                    border: leadScores[lead.id].score === "hot" ? "1px solid rgba(239,68,68,0.3)" : leadScores[lead.id].score === "warm" ? "1px solid rgba(251,191,36,0.3)" : "1px solid rgba(96,165,250,0.3)",
                    color: leadScores[lead.id].score === "hot" ? "#f87171" : leadScores[lead.id].score === "warm" ? "#fbbf24" : "#93c5fd",
                  }}
                >
                  AI {leadScores[lead.id].score}
                </span>
              )}
              <button
                onClick={() => {
  ...
  const renderAnalytics = () => {
    ...
    return (
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2,1fr)" : "repeat(3,1fr)", gap: 12, marginBottom: 12 }}>
          {[
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2,1fr)" : "repeat(3,1fr)", gap: 12, marginBottom: 24 }}>
          {[
            ...
            { label: "Store Visits", value: storeVisits, color: "#60a5fa", sub: "via your link" },
            { label: "Page Views", value: pageViews, color: "#f9a8d4", sub: "pages via your link" },
          ].map(({ label, value, color, sub }) => (
            <div key={label} style={{ background: "#0d1117", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: "14px 16px" }}>
              <p style={{ margin: "0 0 4px", fontSize: 10, color: "#4b5563", textTransform: "uppercase", letterSpacing: "0.06em" }}>{label}</p>
              <p style={{ margin: sub ? "0 0 2px" : 0, fontSize: 28, fontFamily: "'Bebas Neue',sans-serif", color }}>{value}</p>
              {sub && <p style={{ margin: 0, fontSize: 9, color: "#374151" }}>{sub}</p>}
            </div>
          ))}
        </div>
