...
export default function LoginPage() {
  ...
  useEffect(() => {
    setMounted(true);
    // Check if already logged in — just redirect, no profile creation here
  ...
  // ── Simple redirect based on profile role ──────────────────────────────────
  const redirectByRole = async (user) => {
    ...
    const userRole = profile?.role || user?.user_metadata?.role || 'manager';

    if (userRole === 'salesman') {
      navigate('/salesman');
    } else if (userRole === 'superadmin') {
      navigate('/dashboard'); // later change to '/hq'
    } else {
      navigate('/dashboard');
    }
  };
  ...
  const handleRegister = async () => {
    ...
    if (!STRONG_PASSWORD_REGEX.test(regPassword)) {
      setError('Use a strong password: 8+ characters with uppercase, lowercase, number, and special character.');
      return;
    }
    ...
    // Step 3 — Insert profile row directly
    // Only runs if session exists (email confirm disabled)
    // If email confirm is enabled, profile is created on first login via Step 4
    if (data.session?.user) {
      ...
      if (profileError) {
        ...
        // Don't block — auth user exists, profile can be fixed later
      }

      setSuccess('Registration successful! Redirecting...');
      await redirectByRole(data.session.user);
    } else {
      // Email confirmation required — profile will be inserted on first login
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 768px) {
          .login-form-panel { width: 100%; max-width: 520px; border-radius: 18px; }
                ...
                <>
                  ...
                  <div className="field-row">
                    <Field id="ic" label="IC Number" focused={focused}>
                      <TextInput
                        id="ic" placeholder="010203-10-1234" value={ic}
                        onChange={(value) => setIc(formatMalaysianIc(value))}
                        onFocusChange={setFocused} inputMode="numeric" maxLength={14}
                      />
                    ...
                    <Field id="phone" label="Phone Number" focused={focused}>
                      <TextInput
                        id="phone" type="tel" placeholder="+60123456789" value={phone}
                        onChange={(value) => setPhone(formatMalaysianPhone(value))}
                        onFocusChange={setFocused} inputMode="numeric" maxLength={13}
                      />
                      <p className={`phone-hint ${phone !== '+60' ? (isPhoneValid ? 'ok' : 'bad') : ''}`}>
                        Format: +60XXXXXXXXX
                      </p>
                    </Field>
                  </div>
