...
import {
  X, MessageCircle, Save, FileText, PlusCircle,
} from 'lucide-react';
...
function BookingsTab({ userId, listings, salesmen }) {
  ...
  const renderBookingRow = (b) => {
    ...
    const sm = b.profiles;
    const isToday = b.appointment_date && new Date(b.appointment_date).toDateString() === todayStr;
    ...
    return (
      <React.Fragment key={b.id}>
        ...
        <tr
          ...
          style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', background: isToday ? 'rgba(59,130,246,0.03)' : 'transparent', transition: 'background 0.15s' }}
        ...
        {loading ? (
          ...
          <>
            {todaysBookings.length > 0 && (
              <>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 20px 8px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                  <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#3b82f6', boxShadow: '0 0 6px rgba(59,130,246,0.8)', animation: 'hotpulse 1.5s ease-in-out infinite', flexShrink: 0 }} />
                  <span style={{ fontSize: 11, fontWeight: 700, color: '#3b82f6', letterSpacing: '0.12em', textTransform: 'uppercase' }}>Today's Bookings</span>
                  <span style={{ fontSize: 10, fontWeight: 700, background: 'rgba(59,130,246,0.12)', border: '1px solid rgba(59,130,246,0.25)', color: '#93c5fd', borderRadius: 20, padding: '1px 8px' }}>{todaysBookings.length}</span>
                </div>
                <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: "'DM Sans', sans-serif" }}>
                    ...
                    <tbody>{todaysBookings.map(b => renderBookingRow(b))}</tbody>
                  </table>
                </div>
              </>
            )}
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 20px 8px', borderBottom: '1px solid rgba(255,255,255,0.04)', borderTop: todaysBookings.length > 0 ? '1px solid rgba(255,255,255,0.08)' : 'none', marginTop: todaysBookings.length > 0 ? 8 : 0 }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: '#6b7280', letterSpacing: '0.12em', textTransform: 'uppercase' }}>All Bookings</span>
              <span style={{ fontSize: 10, fontWeight: 700, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', color: '#4b5563', borderRadius: 20, padding: '1px 8px' }}>{otherBookings.length}</span>
            </div>
            <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: "'DM Sans', sans-serif" }}>
                <thead className="hidden md:table-header-group">
        ...
        <div className="fixed inset-0 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4" style={{ background: 'rgba(0,0,0,0.78)' }} onClick={() => setReminderTarget(null)}>
          <div className="modal-top rounded-t-2xl sm:rounded-2xl w-full max-w-md overflow-hidden" style={{ background: '#0f1623', border: '1px solid rgba(255,255,255,0.08)' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="p-5 space-y-3">
              <textarea value={reminderMsg} onChange={e => setReminderMsg(e.target.value)} rows={7} className={taCls} placeholder="WhatsApp message…" />
