...
const LEAD_STAGES = [
  ...
  "negotiating", "deposit_taken", "won", "lost",
...
const STAGE_COLOR = {
  ...
  closed_won:   { bg: "rgba(34,197,94,0.18)",   border: "rgba(34,197,94,0.4)",   tx: "#4ade80" },
  closed_lost:  { bg: "rgba(107,114,128,0.12)", border: "rgba(107,114,128,0.3)", tx: "#9ca3af" },
};
...
export default function SalesmanLite() {
  ...
  const [addLeadForm, setAddLeadForm] = useState({ buyer_name: "", phone: "", notes: "", car_listing_id: "", stage: "new" });
  ...
  useEffect(() => {
    ...
    setStaleLeads(
      leads.filter(
        (l) => l.stage !== "won" && l.stage !== "lost" && l.updated_at && new Date(l.updated_at).getTime() < cutoff,
      ),
    );
  }, [leads]);
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data, error }) => {
      ...
      const ROLE_ROUTES = {
        ...
        manager: "/manager", accountant: "/accountant", fi_officer: "/fi",
        admin: "/admin",
      };
      ...
      supabase
        .from("car_listings")
        .select("id, slug, year, brand, model, variant, selling_price, status, images, colour, mileage, transmission")
        .eq("assigned_to", uid)
        .neq("status", "sold")
  ...
  const pingWA = async (lead) => {
    ...
    setLeads((p) => p.map((l) => (l.id === lead.id ? { ...l, updated_at: now } : l)));
  };
  ...
  const handleMerge = async () => {
    ...
    await supabase.from("profiles").update({ dealer_id: data.dealer_id }).eq("id", profile.id);
    await supabase.from("leads").update({ dealer_id: data.dealer_id }).eq("salesman_id", profile.id).is("dealer_id", null);
    await supabase.from("car_listings").update({ dealer_id: data.dealer_id }).eq("assigned_to", profile.id).is("dealer_id", null);
    await supabase.from("dealer_invites").update({ used: true }).eq("code", mergeCode.trim().toUpperCase());
  ...
  const openWACaption = (car) => {
    ...
    if (waPhone) {
      window.open(`https://wa.me/${waPhone.startsWith("6") ? waPhone : "6" + waPhone}?text=${msg}`, "_blank", "noopener,noreferrer");
    } else {
  ...
  const TABS_DESKTOP = [
    { tab: "dashboard", label: "Dashboard", icon: <LayoutGrid style={{ width: 14, height: 14 }} /> },
    { tab: "listings",  label: "My Listings", icon: <Car style={{ width: 14, height: 14 }} />, badge: myListings.length || null },
    { tab: "leads",     label: "Leads", icon: <User style={{ width: 14, height: 14 }} />, badge: leads.filter(l => l.stage !== "lost").length || null },
    { tab: "merge",     label: "Join Dealership", icon: <GitMerge style={{ width: 14, height: 14 }} /> },
  ];
  ...
  const TABS_MOBILE = [
    ...
    { tab: "leads",     label: "Leads",     icon: <User size={18} />, badge: leads.filter(l => l.stage !== "lost").length || null },
  ...
  const renderDashboard = () => {
    const activeLeads = leads.filter((l) => l.stage !== "lost" && l.stage !== "closed_lost");
    const kpis = [
      { label: "Active Leads",   value: activeLeads.length,   color: "#93c5fd" },
      { label: "My Listings",    value: myListings.length,    color: "#4ade80" },
      { label: "Stale Leads",    value: staleLeads.length,    color: "#fb923c", warn: staleLeads.length > 0 },
    ];

    return (
      <div>
        <p style={{ margin: "0 0 16px", fontSize: 16, fontWeight: 600, color: "#f1f5f9" }}>Overview</p>

        {/* KPI cards */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, marginBottom: 24 }}>
          {kpis.map(({ label, value, color, warn }) => (
            <div key={label} style={{ background: "#0d1117", border: `1px solid ${warn ? "rgba(251,146,60,0.3)" : "rgba(255,255,255,0.07)"}`, borderRadius: 10, padding: "14px 16px" }}>
              <p style={{ margin: "0 0 4px", fontSize: 11, color: "#4b5563", textTransform: "uppercase", letterSpacing: "0.06em" }}>{label}</p>
              <p style={{ margin: 0, fontSize: 28, fontFamily: "'Bebas Neue', sans-serif", letterSpacing: "1px", color }}>{value}</p>
            </div>
          ))}
        </div>
        ...
        {/* Stale nudges */}
        {staleLeads.length > 0 && (
          <div style={{ background: "#0d1117", border: "1px solid rgba(251,146,60,0.2)", borderRadius: 10, padding: "14px 16px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
              <AlertCircle size={14} style={{ color: "#fb923c", flexShrink: 0 }} />
              <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: "#fb923c" }}>Follow-up Nudges ({staleLeads.length})</p>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {staleLeads.map((lead) => (
                <div key={lead.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                  <div>
                    <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: "#e5e7eb" }}>{lead.buyer_name || "—"}</p>
                    <p style={{ margin: 0, fontSize: 10, color: "#4b5563" }}>No contact · {timeAgo(lead.updated_at)}</p>
                  </div>
                  {lead.phone && (
                    <button
                      onClick={() => pingWA(lead)}
                      style={{ fontSize: 10, padding: "4px 10px", borderRadius: 6, background: "rgba(37,211,102,0.1)", border: "1px solid rgba(37,211,102,0.2)", color: "#4ade80", cursor: "pointer", flexShrink: 0 }}
                    >
                      WA
                    </button>
                  )}
        ...
        {/* No-dealer banner */}
        <div style={{ marginTop: 20, background: "rgba(37,99,235,0.07)", border: "1px solid rgba(37,99,235,0.2)", borderRadius: 10, padding: "14px 16px" }}>
          <p style={{ margin: "0 0 4px", fontSize: 13, fontWeight: 600, color: "#93c5fd" }}>Upgrade to full panel</p>
          <p style={{ margin: "0 0 10px", fontSize: 12, color: "#4b5563" }}>Ask your dealer for an invite code to unlock all features.</p>
          <button
            onClick={() => setActiveTab("merge")}
            style={{ fontSize: 12, padding: "6px 14px", borderRadius: 7, background: "#2563eb", border: "none", color: "#fff", cursor: "pointer", fontWeight: 600 }}
          >
            Enter Invite Code →
          </button>
        </div>
  ...
  const renderListings = () => (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <p style={{ margin: 0, fontSize: 16, fontWeight: 600, color: "#f1f5f9" }}>My Listings ({myListings.length})</p>
        <button
          onClick={() => setShowAddForm((v) => !v)}
          style={{ display: "flex", alignItems: "center", gap: 6, background: "#1d4ed8", border: "none", borderRadius: 8, color: "#fff", fontSize: 12, fontWeight: 600, padding: "7px 12px", cursor: "pointer" }}
        >
      ...
      {showAddForm && (
        <div style={{ marginBottom: 24, background: "#0d1117", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: 16 }}>
          <CarForm
            onCreate={(car) => {
      ...
      {myListings.length === 0 && !showAddForm && (
        <div style={{ padding: "40px 0", textAlign: "center", color: "#374151" }}>
          ...
          <p style={{ margin: 0, fontSize: 13 }}>No listings yet — add your first car above.</p>
        </div>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {myListings.map((car) => {
          ...
          return (
            <div key={car.id} style={{ background: "#0d1117", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, overflow: "hidden", display: "flex", gap: 0 }}>
              {img && (
              ...
              <div style={{ flex: 1, padding: "12px 14px", minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 6, marginBottom: 4 }}>
                  <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: "#e5e7eb", lineHeight: 1.3 }}>
                    {[car.year, car.brand, car.model].filter(Boolean).join(" ")}
                  </p>
                  <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 99, background: car.status === "available" ? "rgba(34,197,94,0.12)" : "rgba(251,191,36,0.12)", border: `1px solid ${car.status === "available" ? "rgba(34,197,94,0.3)" : "rgba(251,191,36,0.3)"}`, color: car.status === "available" ? "#4ade80" : "#fbbf24", flexShrink: 0, textTransform: "capitalize" }}>{car.status}</span>
                </div>
                {car.selling_price && (
                  <p style={{ margin: "0 0 8px", fontSize: 12, fontWeight: 600, color: "#60a5fa" }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  <button
                    ...
                    style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, padding: "4px 9px", borderRadius: 6, background: copied === "link" ? "rgba(34,197,94,0.1)" : "rgba(255,255,255,0.05)", border: `1px solid ${copied === "link" ? "rgba(34,197,94,0.25)" : "rgba(255,255,255,0.08)"}`, color: copied === "link" ? "#4ade80" : "#6b7280", cursor: "pointer" }}
                  >
                  ...
                  <button
                    ...
                    style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, padding: "4px 9px", borderRadius: 6, background: "rgba(37,211,102,0.08)", border: "1px solid rgba(37,211,102,0.2)", color: "#4ade80", cursor: "pointer" }}
                  >
  ...
  const renderLeads = () => {
    ...
    const renderLeadCard = (lead) => {
      ...
      const nextStage = LEAD_STAGES.filter((s) => s !== "lost" && s !== "won")
      ...
      return (
        <div key={lead.id} style={{ background: "#0d1117", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: "10px 12px" }}>
          ...
          {lead.notes && (
            <p style={{ margin: "0 0 6px", fontSize: 10, color: "#4b5563", fontStyle: "italic", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>"{lead.notes}"</p>
          )}
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
            ...
            {lead.phone && (
              <button onClick={() => pingWA(lead)} style={{ fontSize: 10, padding: "3px 7px", borderRadius: 5, background: "rgba(37,211,102,0.1)", border: "1px solid rgba(37,211,102,0.2)", color: "#4ade80", cursor: "pointer" }}>
                WA
              </button>
            )}
          </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 8 }}>
                {lostLeads.map((lead) => (
                  <div key={lead.id} style={{ background: "#0d1117", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, padding: "10px 12px", opacity: 0.65 }}>
                    <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: "#9ca3af" }}>{lead.buyer_name || "—"}</p>
  ...
  const renderMerge = () => (
    <div style={{ maxWidth: 480 }}>
      <p style={{ margin: "0 0 6px", fontSize: 16, fontWeight: 600, color: "#f1f5f9" }}>Join a Dealership</p>
      <p style={{ margin: "0 0 24px", fontSize: 13, color: "#4b5563", lineHeight: 1.6 }}>
        Got an invite code from your dealer? Enter it below to merge your account into their ShiftOS system.
      </p>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <input
          ...
          style={{ width: "100%", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#e5e7eb", fontSize: 13, padding: "10px 12px", outline: "none", boxSizing: "border-box", fontFamily: "'DM Sans', sans-serif" }}
        />
        <button
          onClick={handleMerge}
          disabled={!mergeCode.trim() || mergeStatus === "pending" || mergeStatus === "success"}
          style={{ padding: "10px 16px", borderRadius: 8, background: mergeStatus === "success" ? "#16a34a" : "#2563eb", border: "none", color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", opacity: (!mergeCode.trim() || mergeStatus === "pending") ? 0.6 : 1 }}
        >
          {mergeStatus === "pending" ? "Verifying..." : mergeStatus === "success" ? "Merged!" : "Request Merge"}
        </button>
      </div>

      {mergeStatus === "error" && (
        <div style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 8 }}>
          <AlertCircle size={14} style={{ color: "#f87171", flexShrink: 0 }} />
          <p style={{ margin: 0, fontSize: 12, color: "#f87171" }}>{mergeMsg}</p>
        </div>
      )}

      {mergeStatus === "success" && (
        <div style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.2)", borderRadius: 8 }}>
          <CheckCircle2 size={14} style={{ color: "#4ade80", flexShrink: 0 }} />
          <p style={{ margin: 0, fontSize: 12, color: "#4ade80" }}>{mergeMsg}</p>
        </div>
      )}

      <p style={{ marginTop: 24, fontSize: 12, color: "#374151", lineHeight: 1.6 }}>
        Don't have a code? Ask your dealer to generate one from their ShiftOS Settings panel.
      </p>
    </div>
  );
          ...
          <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {[
                ...
                { key: "phone",      label: "Phone", placeholder: "e.g. 0123456789" },
                ...
                <div key={key}>
                  ...
                  <input
                    ...
                    onChange={(e) => setAddLeadForm((p) => ({ ...p, [key]: e.target.value }))}
                    placeholder={placeholder}
                    style={{ width: "100%", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#e5e7eb", fontSize: 13, padding: "9px 12px", outline: "none", boxSizing: "border-box", fontFamily: "'DM Sans', sans-serif" }}
                  />
                </div>
              ))}
            </div>
            <button
              ...
              style={{ marginTop: 20, width: "100%", padding: "10px", borderRadius: 8, background: "#2563eb", border: "none", color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", opacity: (!addLeadForm.buyer_name || addLeadSaving) ? 0.6 : 1 }}
            >
  ...
  return (
    <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif", color: "#fff" }}>
      <Helmet>
      ...
      {/* ── Nav ── */}
      {isMobile ? (
        <nav style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 50, height: 60, background: "#080a12", borderTop: "0.5px solid rgba(255,255,255,0.07)", display: "flex" }}>
        ...
        <nav style={{ width: 200, flexShrink: 0, background: "#080a12", borderRight: "1px solid rgba(255,255,255,0.07)", display: "flex", flexDirection: "column", overflowY: "auto", position: "sticky", top: 0, height: "100vh" }}>
          {/* Logo */}
          <div style={{ padding: 16, borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 10, color: "#4b5563", marginTop: 2, marginBottom: 0 }}>· Lite Panel</p>
            </div>
          </div>
          ...
          {TABS_DESKTOP.map(({ tab, label, icon, badge }) => (
            <button key={tab} onClick={() => setActiveTab(tab)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 16px", margin: "1px 8px", borderRadius: 8, cursor: "pointer", background: activeTab === tab ? "rgba(37,99,235,0.15)" : "transparent", border: activeTab === tab ? "0.5px solid rgba(37,99,235,0.25)" : "0.5px solid transparent", color: activeTab === tab ? "#93c5fd" : "#6b7280", fontSize: 13, fontWeight: 500, width: "calc(100% - 16px)", textAlign: "left" }}>
              {icon}
          ...
          <div style={{ marginTop: "auto", borderTop: "1px solid rgba(255,255,255,0.06)", padding: "12px 16px", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <button onClick={handleLogout} style={{ background: "transparent", border: "none", color: "#4b5563", cursor: "pointer", padding: 4 }}>
              <LogOut size={14} />
            </button>
          </div>
        </nav>
            ...
            <>
              <div style={{ flex: 1 }}>
                ...
                <p style={{ margin: 0, fontSize: 12, color: "#64748b", marginTop: 2 }}>
                  {new Date().toLocaleDateString("en-MY", { weekday: "long", day: "numeric", month: "long" })} · Lite Panel
                </p>
              </div>
      ...
      {isMobile && activeTab === "leads" && (
        <button
          onClick={() => setShowAddLead(true)}
          style={{ position: "fixed", bottom: 72, right: 16, zIndex: 40, width: 44, height: 44, borderRadius: "50%", background: "#2563eb", border: "none", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 4px 12px rgba(37,99,235,0.4)" }}
        >
          <Plus size={20} />
        </button>
      )}
