...
> **ACT-1 DONE (verified 2026-08-26)** — TOTP is enabled on the Supabase project. Proof: `auth.mfa_factors` holds one `totp` factor with `status='verified'`, enrolled 2026-06-03 on the Airy Auzy dealer account — a factor cannot be enrolled or verified unless the project-level TOTP factor is on. The Supabase security advisor also no longer raises `auth_insufficient_mfa_options`. This sat in the nag list for ~3 months after the fact. Do NOT enable Supabase's Phone/SMS OTP factor (Bank Negara RMiT bans SMS OTP as a standalone factor; TOTP is the compliant choice). Full enforcement across Google OAuth / magic-link logins is still open — that is ACT-2.
...
> Reminder protocol: while ACT-2, ACT-4, ACT-9 or ACT-10 remain here, surface them at session start and whenever security/auth/import/dependency work is touched. (ACT-3, ACT-5 and NEW-8 completed 2026-08-05. **ACT-8 was found ALREADY COMPLETE and removed 2026-08-15** — `package.json` AND `package-lock.json` both resolve `xlsx` to `https://cdn.sheetjs.com/xlsx-0.20.3/xlsx-0.20.3.tgz`, and `vercel.json` CSP already whitelists `cdn.sheetjs.com` in connect-src; it had been sitting in this list as a blocked user-action for weeks after the fact. **ACT-12 verified RESOLVED 2026-08-24** — see entry above; drop from this nag list. **ACT-1 verified DONE 2026-08-26** — TOTP is on, evidenced by a verified factor in `auth.mfa_factors`; dropped from the nag list. ACT-6 stays deferred until Supabase Pro — the security advisor still reports `auth_leaked_password_protection` disabled, and it is a paid feature, so do not nag.) LESSON: verify an ACT item against the code before re-surfacing it — a stale nag costs a session's attention every time.
