...
- [ ] **DASH-4: Services "Add Product" sidebar dark-on-light + checks** — ServicesPage.jsx:1069-1086 panel uses dark gradient (#0d0d14→#0a0a0f, white text) while app is light; input styles (ServicesPage.jsx:325-337) use `color:white`. FIX: light theme matching LeadDrawer.jsx:895-902 (#fff bg, #e5e7eb border, #111827/#6b7280 text). Also: clarify the "sold with add-ons should count as sold" metric (need user to confirm which number looks wrong — current wonCount counts all won leads regardless of add-ons). Stock count already implemented (track_stock toggle).
