...
### DEALER SUBDOMAIN — XDRIVE SITE AUDIT (2026-06-10)

- [x] **SITE-1 (CRITICAL): /showroom dead end on subdomain** — HeroCarousel "Browse Our Cars" CTA, hero search submit, Footer "Browse Cars" link, and CarDetailPage "not found" back-link all hardcoded `/showroom`. On subdomain `/showroom` → splash → redirect to homepage (never reaches listings). FIXED: all 4 hardcoded links now use `isSubdomain() ? "/cars" : "/showroom"`.
- [x] **SITE-6 (HIGH): Dealer name shows as "Dealer" on all listing pages** — CarDetailPage fetches dealer profile via `public_dealer_profiles` view (security_invoker=true, no anon RLS). Anonymous visitors get null → dealer card falls back to "Dealer". FIXED: switched to `get_dealer_profile_by_id` SECURITY DEFINER RPC.
- [x] **SITE-11 (MEDIUM): analytics_events returning 400 errors** — CarDetailPage:539 and HomePage:222 bypass `trackEvent` util and insert with `metadata` field directly; also missing `session_id`. FIXED: replaced both with `trackEvent()` calls.
- [ ] **SITE-2 (CRITICAL): 3+ Supabase storage images returning 503** — Storage bucket is full (see INFRA-1). Hero carousel Honda Civic Type-R shows only alt text. USER ACTION: audit and clear orphaned images in Supabase Storage dashboard, then re-upload broken images.
- [ ] **SITE-3 (HIGH): Corrupted test text in storefront content** — "Why Choose Us" and "How It Works" headings contain keyboard-mash test text (e.g. "daw a", "testseTell...adawd"). These are editable fields stored in the dealer profile. USER ACTION: fix via Dashboard → Settings → Storefront Content tab.
- [ ] **SITE-4 (HIGH): About section on listing pages shows "G"** — car.specs field is set to "G" (single letter) for one or more listings; the About section shows specs verbatim. USER ACTION: edit the affected listing(s) and fill in a proper description in the Specs/Description field.
- [ ] **SITE-5 (HIGH): Personal Gmail address exposed in footer** — Footer contact email shows `Auzyvyth@gmail.com`. USER ACTION: update via Dashboard → Settings → Contact/Profile (or wherever email is stored in profiles).
- [ ] **SITE-7 (MEDIUM): Facebook link is a test placeholder** — Footer shows `facebook.com/test`. USER ACTION: update Facebook URL in Dashboard → Settings → Social Links.
- [ ] **SITE-8 (MEDIUM): 5–7 second black screen before content** — React SPA with no SSR or skeleton loading; Supabase fetch waterfall. Tracked as PERF-2 below (tenant resolution blocks listings fetch).
- [ ] **SITE-9 (MEDIUM): `get_dealer_profile_by_subdomain` called 7x per load** — tracked as PERF-3 below.
- [ ] **SITE-10 (LOW): "2+" cars sold stat** — actual count from stock_units; low because this is a new account. No code fix needed — will rise as sales are recorded.
...
- [x] **PERF-3 (HIGH): Triple-redundant dealer-profile fetch** — useTenant.js:81-85 (full PROFILE_SELECT incl. whatsapp_number), useCTAContext.js:53-57, and HomePage.jsx:205-212 (whatsapp_number again) all hit public_dealer_profiles for the same dealer. FIXED: deleted the redundant HomePage whatsapp fetch; tenant.whatsapp_number used directly.
- [x] **PERF-4 (MEDIUM): Redundant per-row dealer join in listings query** — HomePage.jsx:255 joins dealer:profiles(...) onto all 30 rows even though every row is the same dealer (already in `tenant`). FIXED: skip the join when tenant?.id is set; attach tenant as the dealer object client-side post-fetch.
- [x] **PERF-5 (LOW): Sold-count stat delayed by flat 800ms timer** — HomePage.jsx:298 `setTimeout(fetchSoldCount, 800)` instead of firing in parallel with load(). FIXED: Promise.all alongside load().
