...
const SITE_DESCRIPTION = 'Browse quality used cars on XDrive — Malaysia's trusted car marketplace.';

const BOT_PATTERNS = [
  'googlebot', 'bingbot', 'facebookexternalhit', 'whatsapp',
  'telegrambot', 'twitterbot', 'discordbot', 'linkedinbot',
  'slurp', 'duckduckbot', 'yandex',
];
...
export default async function handler(req) {
  ...
  if (carMatch) {
    ...
    const { data: car } = await supabase
      .from('car_listings')
      .select('brand, model, variant, year, selling_price, mileage, colour, images, city, state, transmission, fuel_type')
      .eq('slug', slug)
      .eq('status', 'active')
      .maybeSingle();
    ...
    if (car) {
      ...
      const image = (Array.isArray(car.images) && car.images[0]) ? car.images[0] : DEFAULT_OG_IMAGE;

      return new Response(buildHtml({ title, description, image, canonicalUrl }), {
        status: 200,
