-- Fills maker_canon on insert so model_key (generated from it) matches keyOf().
...
begin
  ...
  insert into public.reg_car_month (month, body_type, maker, maker_canon, model, colour, fuel, n)
  select date_trunc('month', a[1]::date)::date,
         a[2], a[3], coalesce(al.canonical, a[3]), a[4], a[5], a[6], count(*)::integer
    from _reg_rows
    left join public.reg_maker_alias al on al.source_maker = a[3]
