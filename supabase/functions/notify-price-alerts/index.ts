 ...
 * notify-price-alerts
 *
 * Runs on a schedule (every 30 min via pg_cron, jobid 3).
 ...
 * the first repo copy — it carries a fix for a real bug (below), not a
 * blind copy-down of the deployed version. Diff against
 * `mcp__Supabase__get_edge_function` before touching it again.
 *
 * BUG FIXED 2026-08-24: matchesAlert() never checked `alert.keyword`. Every
 * alert saved with ONLY a keyword (e.g. "Honda Civic", "Ferrari") — which is
 * every alert saved from the showroom search box — had brand/model/variant/
 * etc all null, so it matched every single new listing regardless of make.
 * All 4 live alerts were digest-spamming their owner on every new listing of
 * any car, every 30 minutes. matchesKeyword() below replicates the same
 * tokenized AND-match CarListingPage.jsx uses for its search box (each
 * whitespace-split token must appear in brand/model/variant), scoped to the
 * columns this function already selects.
 */
...
// Same shape as the showroom search box (CarListingPage.jsx): every
// whitespace-split token must appear in brand, model or variant. Options/
// features/specs aren't selected here, so this is scoped to car identity —
// good enough for "which car is this alert about", which is all a keyword
// alert (no brand/model/variant fields set) is really asking.
function matchesKeyword(car: Record<string, unknown>, keyword: unknown): boolean {
  const kw = String(keyword || '').trim();
  if (!kw) return true;
  const tokens = kw.toLowerCase().split(/\s+/).filter(Boolean).slice(0, 6);
  const hay = [car.brand, car.model, car.variant].filter(Boolean).join(' ').toLowerCase();
  return tokens.every((t) => hay.includes(t));
}

function matchesAlert(car: Record<string, unknown>, alert: Record<string, unknown>): boolean {
  if (!matchesKeyword(car, alert.keyword)) return false;
...
function buildEmailHtml(cars: Record<string, unknown>[], alert: Record<string, unknown>): string {
  ...
  const showroomUrl = (() => {
    ...
    if (alert.keyword)   p.set('q',         alert.keyword as string);
